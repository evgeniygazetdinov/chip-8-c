        ...
        case 0x8000:
            switch (n)
            {
                ...
                case 0x4:
                    V[0xF] = (V[x] + V[y] > 0xFF)
                                 ? 1
                                 : 0; // SET VX=VX+VY VF=move 8xy4
                    V[x] += V[y];
                    break;
                case 0x5:
                    V[0xF] = (V[x] > V[y])
                                 ? 1
                                 : 0; // 8xy5 set VX=VX+XY set VF= not zaem
                    V[x] -= V[y];
                    break;
                case 0x6:
                    V[0xF] = V[x] & 0x1; // 8xy6  move VX to left by 1 /VF yound
                                         // bit before sdvig
                    V[x] >>= 1;
                    break;
                case 0x7:
                    V[0xF] = (V[y] > V[x])
                                 ? 1
                                 : 0; // 8xy7 set Vx=VY-VX set vf = not zaem
                    V[x] = V[y] - V[x];
                    break;
                case 0xE:
                    V[0xF] = (V[x] >> 7) & 0x1; // 8xyE moving VX left by one,
                                                // VF=ELDER bit before sdvig
                    V[x] <<= 1;
                    break;
                default:
        ...
        case 0xF000:
            switch (kk)
            {
                ...
                case 0x29: // Fx29: set  IND = adreress form for symbol Vx
                    IND = 0x50 +
                          (V[x] * 5); // fonts begin from 0x50, each by 5 bites
                    break;
    ...
    while (running)
    {
        ...
        if (font)
        {
            ...
            SDL_Surface* text_surface =
                TTF_RenderText_Blended(font, "Hello, SDL2 World!", white);
            ...
            if (text_surface)
            {
                ...
                if (text_texture)
                {
                    ...
                    SDL_Rect text_rect = {(800 - text_width) / 2,
                                          (600 - text_height) / 2, text_width,
                                          text_height};
